// $Header: //depot/release/jawsSP1/develop/global/src/dbxsdk/samples/dumpdwg/dumpdwg.cpp#1 $ $Change: 313499 $ $DateTime: 2012/05/08 21:32:31 $ $Author: integrat $
// $NoKeywords: $
//
//////////////////////////////////////////////////////////////////////////////
//
...
Acad::ErrorStatus  
DumpDwgHostApp::getRemoteFile(const TCHAR* pszURL, TCHAR* pszLocalFile, size_t nFilePath, Adesk::Boolean bIgnoreCache) const
{
    ...
    for (i = 1; i < argc; i++)
    {
        ...
        if (argv[i][0])
        {
            if (acdbHostApplicationServices()->isURL(argv[i]))
            {
                ...
                Acad::ErrorStatus es = acdbHostApplicationServices()->getRemoteFile(argv[i], szLocalFile, false);
                if ((Acad::eInetOk == es || Acad::eInetInCache == es))
                    readDrawing(szLocalFile);
                else
                    _tprintf(_T("Not Found or Cannot be Downloaded: %s\n"), argv[i]);
            }
            else
            {
                DirEnum de;

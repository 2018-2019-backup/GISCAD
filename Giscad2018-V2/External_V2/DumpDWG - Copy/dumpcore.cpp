// $Header: //depot/release/jawsSP1/develop/global/src/dbxsdk/samples/dumpdwg/dumpcore.cpp#1 $Change: 78235 $ $DateTime: 2012/05/08 21:32:31 $ $Author: integrat $
// $NoKeywords: $
//
//////////////////////////////////////////////////////////////////////////////
//
            ...
            if (pRecord->getName(pName) == Acad::eOk)
            {
                ...
                else
                {
                    ...
                    if (es == Acad::eOk)
                    {
                        ...
                        _print(_T("it's really a %s try a cast <%Ix>\n"), p,
                            AcDbLinetypeTableRecord::cast(pObject));

                        pObject->close();
                    }
                }
                _print(_T("        %s color %d lt %s\n"), 
        ...
        if (pIter->getRecord(pRecord, AcDb::kForRead) == Acad::eOk)
        {
            ...
            if (pRecord->getName(pName) == Acad::eOk)
            {
                ...
                if (Tables)
                {
                    ...
                    _print(_T("\t\tAcad::ErrorStatus fileName(char*&) %s\n"), 
                        pStr);
                    pRecord->bigFontFileName(pStr);
                    _print(_T("\t\tAcad::ErrorStatus bigFontFileName(char*&) %s\n"), 
                        pStr);
                }
            }
            pRecord->close();
    ...
    while (!pIter->done())
    {
        if (pIter->getRecord(pRecord, AcDb::kForRead) == Acad::eOk)
        {
            ...
            if (pRecord->getName(pName) == Acad::eOk)
            {
                ...
                _print(_T("\tAcad::ErrorStatus getDimpost(char*&) %s\n"), pStr);
                pStr = str;
                pRecord->getDimapost(pStr);
                _print(_T("\tAcad::ErrorStatus getDimapost(char*&) %s\n"), pStr);
                pStr = str;
                pRecord->getDimblk(pStr);
                _print(_T("\tAcad::ErrorStatus getDimblk(char*&) %s\n"), pStr);
                pStr = str;
                pRecord->getDimblk1(pStr);
                _print(_T("\tAcad::ErrorStatus getDimblk1(char*&) %s\n"), pStr);
                pStr = str;
                pRecord->getDimblk2(pStr);
                _print(_T("\tAcad::ErrorStatus getDimblk2(char*&) %s\n"), pStr);
...
void dumpAcDbEntity(AcDbEntity *pEnt)
{
    ...
    {
        _print(_T("\t\tcolor %s layer %s linetype %s linetypeScale %g\n"),
            pEnt->colorIndex() == 0 ? _T("BYBLOCK") : _T("BYLAYER"), 
            pLayer, pLinetype, pEnt->linetypeScale());
    }
    else
    {
        _print(_T("\t\tcolor %d layer %s linetype %s linetypeScale %g\n"),
    ...
    if (pEntity)
    {
        ...
        _print(_T("acdbopenacdbentity: %s\n"), acadErrorStatusText(es));
#endif
    ...
    if (pBuf)
    {
        ...
        while (p)
        {
            ...
            case AcDb::kDxfXdAsciiString:
                _print(_T("\t\t\tAsciiString: %s\n"), p->resval.rstring);
                break;
            case AcDb::kDxfRegAppName:
                _print(_T("\t\t\tRegAppName: %s\n"), p->resval.rstring);
                break;
            case AcDb::kDxfXdControlString:
                _print(_T("\t\t\tControlString: %s\n"), p->resval.rstring);
                break;
            case AcDb::kDxfXdLayerName:
                _print(_T("\t\t\tLayerName: %s\n"), p->resval.rstring);
                break;
...
void dumpAcDbMline(AcDbEntity *pEnt)
{
    ...
    for (i = 0; i < numVertices; i++)
    {
        ...
        {
            AcGeDoubleArray* pElementParameters = (AcGeDoubleArray*)params[j];
...
void dumpAcDbMText(AcDbEntity *pEnt)
{
    ...
    {
        AcGePoint3d pt = boundingPoints.at(i);
        _print(_T("\t\t\t%g,%g,%g\n"),
            pt.x, pt.y, pt.z);
    }
...
void dumpAcDbProxyEntity(AcDbEntity *pEnt)
{
    ...
    _print(_T("\t\tDXF name:\t %s\n"), outstr);
    outstr = pAcDbProxyEntity->originalClassName();
    _print(_T("\t\tClass name:\t %s\n"), outstr);
}
...
void deleteEntMap()
{
    ...
    {
        EntInfo *pInfo = 0;
        for (void* pos = entMap->GetStartPosition();pos != NULL;)
        {
...
void dumpDwgHeaderVars(AcDbDatabase *pDatabase)
{
    ...
    _print(_T("\tAcad::ErrorStatus getMenu(char*&) %s\n"), pStr);
    ...
    _print(_T("\tAcad::ErrorStatus getDimpost(char*& pOutput) %s\n"), pStr);

    // Acad::ErrorStatus getDimapost(char*& pOutput)
    pDatabase->getDimapost(pStr);
    _print(_T("\tAcad::ErrorStatus getDimapost(char*& pOutput) %s\n"), pStr);

    // Acad::ErrorStatus getDimblk(char*& pOutput)
    pDatabase->getDimblk(pStr);
    _print(_T("\tAcad::ErrorStatus getDimblk(char*& pOutput) %s\n"), pStr);

    // Acad::ErrorStatus getDimblk1(char*& pOutput)
    pDatabase->getDimblk1(pStr);
    _print(_T("\tAcad::ErrorStatus getDimblk1(char*& pOutput) %s\n"), pStr);

    // Acad::ErrorStatus getDimblk2(char*& pOutput)
    pDatabase->getDimblk2(pStr);
    _print(_T("\tAcad::ErrorStatus getDimblk2(char*& pOutput) %s\n"), pStr);

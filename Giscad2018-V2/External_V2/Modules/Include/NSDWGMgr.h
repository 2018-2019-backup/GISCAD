...
public:
	...
	int openStandardDWG(TCHAR* pszFileName, bool bOpenExisting);
	...
	//Added on 6th July 2019 for Bulk drawing issue
	int getSTDLayerProperties(std::map<NSSTRING, CNSLayer*> &layerProperties);
	int getSTDTextStyleProperties(std::map<NSSTRING, CNSTextStyle*> &TextStyleProperties);
	int getSTDLineTypeList(std::map<NSSTRING, CNSLineType*> &LineTypeProperties);
	int getSTDDimStyleList(std::map<NSSTRING, CNSDimStyle*> &DimStyleProperties);
	int checkSTDBlockAttributeVisibility(std::map<NSSTRING, std::map<NSSTRING, CNSAttributeRef*>*>&mapBlockAttrVisibility);

	// Validation functions
	int getFeaturesToBeValidated(std::map<LONG_PTR, CNSValidationDataCollector*> &mapBlocksToBeValidated, std::map<LONG_PTR, CNSValidationDataCollector*> &mapLinearFeaturesToBeValidated, bool bGetXInfo, bool getLinerFeatures, TCHAR* pszLayerNameSuffix);
	int addXDATA(LONG_PTR lObjectId, std::map<NSSTRING, NSSTRING> &mapXData);
	int getFeaturesOnLayer(std::map<LONG_PTR, CNSValidationDataCollector*> &mapBlocksToBeValidated, std::map<LONG_PTR, CNSValidationDataCollector*> &mapLinearFeaturesToBeValidated, int nRelationshipFlag, TCHAR* pszLayerName);
	...
	int getPlineRecord(LONG_PTR ObjectId, CNSPoint3D &pointStart, CNSPoint3D &pointVertex, double &dArea);
	int selectNonStdBlocks(std::map<LONG_PTR, CNSValidationDataCollector*> &mapBlocksToBeValidated);
	int isTypeStd(NSSTRING strExtName);
	int GetBlockObjectsWithInsertionPoint(std::map<AcDbObjectId,AcGePoint3d> &blockData);
    int GetInsertionPoint(LONG_PTR nObjectID, CNSPoint3D &pt);
	...
	int getPromptValues(LONG_PTR lblkTblID, std::map<NSSTRING, NSSTRING> &mapofPrompValues);
...
private:
	...
	//Added on 6th July 2019 for Bulk drawing issue
	AcDbDatabase* m_pStdDWGDatabase;
...
public:
	int getBlocksOnNonMatchingLayer(std::map<LONG_PTR, CNSValidationDataCollector*> &mapBlocksOnWrongLayer,  TCHAR* pszNameSuff);

...
#define NPI ((double)3.14159265358979323846)
...
static const TCHAR acIsmObjDbxFile[] = _T("acismobj22.dbx");
...
public:
    ...
    Adesk::Boolean isRemoteFile(const TCHAR* pszLocalFile, TCHAR* pszURL, size_t nUrlSize) const override;
    Acad::ErrorStatus  getRemoteFile(const TCHAR* pszURL, TCHAR* pszLocalFile, size_t nFilePath, Adesk::Boolean bIgnoreCache) const override;
...
const ACHAR * CreatentHostApp::getMachineRegistryProductRootKey ()
{
    return registryRoot;
}
#endif

CreatentHostApp::~CreatentHostApp()
{
    CString local,url;
...
Adesk::Boolean 
CreatentHostApp::isURL(const TCHAR* pszURL) const
{
    return PathIsURL(pszURL);
}

Adesk::Boolean 
CreatentHostApp::isRemoteFile(const TCHAR* pszLocalFile, TCHAR* pszURL, size_t nUrlSize) const
{
    CString value;
...
Acad::ErrorStatus  
CreatentHostApp::getRemoteFile(const TCHAR* pszURL, TCHAR* pszLocalFile, size_t nFilePath, Adesk::Boolean bIgnoreCache) const
{
...
bool CreatentHostApp::notifyCorruptDrawingFoundOnOpen(AcDbObjectId id, Acad::ErrorStatus es)
{
...
class CreatentHostApp ;
/*
Validate - Entity Filter Added in below functions - Kumanan - 19-05-2015
---------------------------------------
getFeaturesToBeValidated()
getFeaturesOnLayer()
getTextHeight()
getGeometryOnZeroLayer()
selectNonStdBlocks()
GetBlockObjectsWithInsertionPoint()
GetLineObjects()
eraseGISErrorLayer()
getBlocksOnNonMatchingLayer()

Convert to Xml - Kumanan - 20-05-2015
-------------------
listTitleBlockHeaderAttributes()
selectBlocks()
selectDeletedBlocks()
selectPolyLines()
selectTextLabelsOnDP()

			AcDbObjectId entId;
		Acad::ErrorStatus es = pBlkTblRcdItr->getEntityId(entId);
	
		if (!(entId.objectClass()->isDerivedFrom( AcDbCircle::desc()) ||
			entId.objectClass()->isDerivedFrom( AcDbPolyline::desc()) ||
			entId.objectClass()->isDerivedFrom( AcDbLine::desc()) ||
			entId.objectClass()->isDerivedFrom( AcDbArc::desc()) ||
			entId.objectClass()->isDerivedFrom( AcDbBlockReference::desc()) ||
			entId.objectClass()->isDerivedFrom( AcDbMline::desc())
			))
			continue;
...
int resolveXrefs = 0;
bool bCorruptDrawing = false;
static const TCHAR acIsmObjDbxFile[] = _T("acIsmObj19.dbx");
...
public:
    ...
    //URL related services
    Adesk::Boolean isURL(const TCHAR* pszURL) const;
    Adesk::Boolean isRemoteFile(const TCHAR* pszLocalFile, TCHAR* pszURL, size_t nUrlSize) const;
    Acad::ErrorStatus  getRemoteFile(const TCHAR* pszURL, TCHAR* pszLocalFile, size_t nFilePath, Adesk::Boolean bIgnoreCache) const;
...
//Added for Bulk drawing issue on 11 July 2019
bool g_bIsFirstTime = true;
extern "C"
{
	__declspec(dllexport) void ReleaseRealDWG();
};

__declspec(dllexport) void __cdecl ReleaseRealDWG()
{
	if (g_bIsFirstTime == false)
	{
...
CNSDWGMgr::CNSDWGMgr(void)
{
	...
	//AfxMessageBox(L"10");
	if (g_bIsFirstTime == true)
	{
		es = acdbSetHostApplicationServices(&gCreatentHostApp);
		//AfxMessageBox(L"11");
		long lcid = 0x00000409;  // English
		//AfxMessageBox(L"12");
		es = acdbValidateSetup(lcid);
		//AfxMessageBox(L"13");
	...
	//Added on 3rd July 2019 for Bulk drawing issue
	m_pStdDWGDatabase = NULL;
	m_dTextHeight = 2.5;
}
...
CNSDWGMgr::~CNSDWGMgr(void)
{
	...
	//Added on 6th July 2019 for Bulk drawing issue
	if (m_pStdDWGDatabase != NULL)
	{
		delete m_pStdDWGDatabase;
		m_pStdDWGDatabase = NULL;
	}
	  //acrxUnloadModule(acIsmObjDbxFile);
#endif
...
//{
//    CNSDWGMgr* pObject = (CNSDWGMgr*)pParam;
//	#ifndef _INTERNAL_
...
//	//	return NS_FAIL;
//	AcDbDatabase *pDB ;
//	pObject->getSourceDatabase(pDB);
//	Acad::ErrorStatus es = pDB->readDwgFile(L"c:\\xyz.dwg");
//
//#endif
//   // if (pObject == NULL ||
...
//Added on 6th July 2019 for Bulk drawing issue
//To open standard template file
int CNSDWGMgr::openStandardDWG(TCHAR* pFileName, bool bOpenExisting)
{
	m_pStdDWGDatabase = new AcDbDatabase(Adesk::kFalse);
	if (m_pStdDWGDatabase == NULL)
		return NS_FAIL;
	...
	if (_tcsicmp(ext, _T(".dwg")) && _tcscmp(ext, _T(".dxf")) && _tcscmp(ext, _T(".dwt")))
	{
		_tprintf(_T("Can't open %s\n"), pFileName);
		_tprintf(_T("File must have .dwg or .dxf extension\n"));
		delete m_pStdDWGDatabase;
	...
	{
		Sleep(1000);
		if ((_tcsicmp(ext, _T(".dwg")) && _tcsicmp(ext, _T(".dwt")) ? m_pStdDWGDatabase->dxfIn(pFileName)
			: m_pStdDWGDatabase->readDwgFile(pFileName)) == Acad::eOk
			/*&& !bCorruptDrawing*/)
...

		{
			assert(Copy != 2);   // no more saveasr13 support
			assert(Copy != 3);   // no more saveasr14 support
			acdbHostApplicationServices()->setWorkingDatabase(m_pStdDWGDatabase);
			if (resolveXrefs)
				acdbResolveCurrentXRefs(m_pStdDWGDatabase);
			//dumpDatabase(m_pStdDWGDatabase);
			if (Copy == 1)
				saveDwg(m_pStdDWGDatabase);
			else if (Copy == 4)
			{
				m_pStdDWGDatabase->closeInput(true);
				saveAsR2000Dwg(m_pStdDWGDatabase);
			}
			//dumpEntInfo();
		}
		else
...
int CNSDWGMgr::openDWG(TCHAR* pFileName, bool bOpenExisting)
{
	...
	//MessageBox(NULL, _T("Open - 1"), _T("Check"), MB_OK);
    // Read the drawing  or dxf file.
	try
	{
		Sleep(1000);
		if ((_tcsicmp(ext, _T(".dwg")) && _tcsicmp(ext, _T(".dwt")) ? m_pCurrentDatabase->dxfIn(pFileName)
			: m_pCurrentDatabase->readDwgFile(pFileName)) == Acad::eOk
			/*&& !bCorruptDrawing*/)


		{
			//Sleep(1000);
			//MessageBox(NULL, _T("Open - 2"), _T("Check"), MB_OK);
			assert(Copy != 2);   // no more saveasr13 support
			assert(Copy != 3);   // no more saveasr14 support
			acdbHostApplicationServices()->setWorkingDatabase(m_pCurrentDatabase);
			if (resolveXrefs)
				acdbResolveCurrentXRefs(m_pCurrentDatabase);
			//dumpDatabase(m_pCurrentDatabase);
			if (Copy == 1)
				saveDwg(m_pCurrentDatabase);
			else if (Copy == 4)
			{
				m_pCurrentDatabase->closeInput(true);
				saveAsR2000Dwg(m_pCurrentDatabase);
			}
			//dumpEntInfo();
		}
		else
	...
	//MessageBox(NULL, _T("Open - 3"), _T("Check"), MB_OK);
    //if (resolveXrefs)
...
//			{
//				NSSTRING verDWG=getDWGVersionInfo((const wchar_t*)p_szFileName);
//				//if(verDWG==L"AutoCAD 2013")
//				//if(1)
//				pFileName=p_szFileName;
//				//AfxBeginThread(MyThreadProc, this);
//				es = m_pCurrentDatabase->readDwgFile(p_szFileName);
//				//else
//				//{
//				//	//MessageBox(NULL, _T("drawing version is not 2013/2014"), _T("version"), MB_OK);
//				//	//WriteToLog(LOG_OTHER, _T("drawing version is not 2013/2014"));
//				//	//NSSPRINTF(szFormat, _T("Error in opening drawing file:\'%s\'"), strFileName.c_str());
//				//	WriteErrorRec(LOG_OTHER,  _T("drawing version is not 2013/2014"));//WriteToLog(LOG_OTHER, szFormat);
//				//	return NS_FAIL;
//				//}
//					
...
//		{
//			////Kumanan
//			////Code for checking drg header information
...
//			//listTitleBlockHeaderAttributes(m_mapOfTitleAttributes, mapOfPromptValues);
//
...
int CNSDWGMgr::addNewTextStyle(TCHAR* p_szStyleName, TCHAR* p_szFontName)
{
	...
	if(Acad::eOk != pTableRecord->setFont(p_szFontName,FALSE,FALSE, (Charset)200, Autodesk::AutoCAD::PAL::FontUtils::FontPitch::kDefault, Autodesk::AutoCAD::PAL::FontUtils::FontFamily::kDecorative)) // Set font name
	{
		pTableRecord->close();
		pTable->close();
		return NS_FAIL;
...
int CNSDWGMgr::ReleaseHostDwg()
{
	...
	acdbReleaseHostDwg(m_pCurrentDatabase);	
	//Added on 6th July 2019 for Bulk drawing issue
	acdbReleaseHostDwg(m_pStdDWGDatabase);
	//acdbCleanUp();
	return NS_SUCCESS;
}

int CNSDWGMgr::XrefData()
{
	AcDbXrefGraph graph;
	Acad::ErrorStatus es=acdbGetHostDwgXrefGraph(m_pCurrentDatabase, graph);
...
int CNSDWGMgr::getBlock(const TCHAR* p_szBlkname, AcDbObjectId& blockId)
{
	...
	Acad::ErrorStatus es =pBlockTable->getAt(p_szBlkname, pBlkTableRecord, AcDb::kForRead);
	if(Acad::eOk != es) // get block table record of the given blockname
	{
		pBlockTable->close();
		return NS_FAIL;
	}
...
int CNSDWGMgr::setTitleBlockAttributes(std::map<NSSTRING,NSSTRING> mAttributes,int xmlConversion)
{
	if(xmlConversion ==0)
	{
	...
	else
	{
		...
		for(;!dicIter->done();dicIter->next())
		{
			...
			layout->close();
			es = acdbOpenObject(pBlockTableRecord,layoutId,AcDb::kForRead);
			AcDbBlockTableRecordIterator *pBlockIterator;
			pBlockTableRecord->newIterator(pBlockIterator);
			for (; !pBlockIterator->done();pBlockIterator->step())
			{
				...
				for(pIter->start();!pIter->done();pIter->step())
				{
					...
					AcDbAttribute *pAtt;
							//pAcDbBlockReference->upgradeOpen();
					if (pAcDbBlockReference->openAttribute(pAtt, objId, AcDb::kForWrite)== Acad::eOk)
					{
						// code for checking if attribute is present in the map and then setting value should come here            
						const TCHAR* szAttName = pAtt->tag();
						...
						for(it = mAttributes.begin(); it != mAttributes.end(); it++)
						{
							...
							{
								pAtt->upgradeOpen();
								//basePoint += pAcDbBlockReference->position().asVector();
								AcGePoint3d basePoint = pAtt->position();
								AcGePoint3d alnPoint = pAtt->alignmentPoint();
								//double widFact = pAtt->widthFactor();
								TCHAR attrText[255];
								_tcscpy_s(attrText, 255, ((*it).second).c_str() );
								pAtt->setTextString(attrText);
								//pAtt->setHorizontalMode(AcDb::kTextMid);
								...
								//
									...
									AcGePoint3d basePoint2 = pAtt->position();
								//	basePoint2 += pAcDbBlockReference->position().asVector();
								//									//pAtt->recordGraphicsModified();
								////pAtt->setLockPositionInBlock(false);
								AcGePoint3d alnPoint2 = pAtt->alignmentPoint();
								//double widFact2 = pAtt->widthFactor();
								//AcDbExtents bounds;
								//pAtt->bounds(bounds);
								int i=0;
								//pAtt->downgradeOpen();
								//pAtt->setPosition(pAtt->alignmentPoint());
								//pAtt->adjustAlignment();
								//pAtt->upgradeOpen();
								//pAcDbBlockReference->upgradeOpen();
								//pBlockTableRecord->upgradeOpen();
								//resbuf *res=acdbEntGet(pAcDbBlockReference);
							}
						}
						//pAtt->getStretchPoints();
					...
					AcDbAttribute *pAtt2;
					if (pAcDbBlockReference->openAttribute(pAtt2, objId, AcDb::kForRead)== Acad::eOk)
					{
						const TCHAR* szAttName = pAtt2->tag();
						std::map<NSSTRING,NSSTRING>::iterator it;
						for(it = mAttributes.begin(); it != mAttributes.end(); it++)
						{
							if(NSSTRCMP(szAttName, ((*it).first).c_str()) == 0)
							{
								TCHAR szFormat[_MAX_PATH + 1];
								NSSPRINTF(szFormat,L"XML-%s,%d,%d,%d",szAttName,pAtt2->horizontalMode(),
										pAtt2->verticalMode(),pAtt2->justification());
		...
		//pAcDbBlockReference->downgradeOpen();
				//AcGeScale3d s3d(1.0,1.0,1.0);
				//pAcDbBlockReference->setScaleFactors(s3d);
				//const AcGeScale3d scale(1.0,1.0,1.0);//=  pAcDbBlockReference->scaleFactors();
				//pAcDbBlockReference->setScaleFactors(scale);


				//pAcDbBlockReference->upgradeFromNotify();
				delete pIter;

			//	AcGeMatrix3d mat;
   //mat = mat.scaling(1.0, AcGePoint3d(0,0,0));
   //   pAcDbBlockReference->transformBy(mat);

	  //AcGePoint3d pt(100,0,0);
	  
		// pAcDbBlockReference->setPosition(pt);
	
	  //Adesk::Boolean canWrite = Adesk::kTrue;
   //pAcDbBlockReference->upgradeFromNotify(canWrite);
	
   // ads_name adsName;
   //acdbGetAdsName(adsName,blockId);
      //acdbEntUpd(adsName);
...
int CNSDWGMgr::listTitleBlockHeaderAttributes(std::map<NSSTRING, NSSTRING>& mapTableAttributes2, std::map<NSSTRING, NSSTRING> &mapofPrompValues)
{
	AcDbBlockTable* pBTable = NULL;
	m_pCurrentDatabase->getBlockTable(pBTable, AcDb::kForRead);
	...
	for(;!p_Iter->done();p_Iter->step())
	{
		...
		if (!objId.objectClass()->isDerivedFrom(AcDbBlockReference::desc()))
			continue;
		...
		if (pTitleBlockReference.openStatus() == Acad::eOk)
		{
			...
			for(pBTRItr->start(); !pBTRItr->done(); pBTRItr->step())
			{
				...
				AcDbObjectId entId;
		Acad::ErrorStatus es = pBTRItr->getEntityId(entId);
		if (entId.objectClass()->isDerivedFrom(AcDbAttributeDefinition::desc())) 
				//if (Acad::eOk ==pBTRItr->getEntity(pAttDefEnt, AcDb::kForRead))
				{
					es=acdbOpenObject(pAttDefEnt, entId, AcDb::kForRead);
					if(pAttDefEnt == NULL)
					{
						continue;
					}
			...
			for(pIter->start(); !pIter->done(); pIter->step())
			{
				...
				for(itrMap = mapTableAttributes2.begin(); itrMap != mapTableAttributes2.end(); itrMap++)
				{
...
int CNSDWGMgr::addXDATA(LONG_PTR lObjectId, std::map<NSSTRING, NSSTRING> &mapXData)
{
	Acad::ErrorStatus es = Acad::eOk;
	AcDbObjectId ObjectId;
	ObjectId.setFromOldId((Adesk::UIntPtr) lObjectId);
...
int CNSDWGMgr::selectBlocks(std::vector<CNSFeatureContainer*>& vectBlocks, TCHAR* pszLayerName, TCHAR* pszBlockName)
{
	...
	/*	if(Acad::eOk != pBlkTblRcdItr->getEntity(pEnt,AcDb::kForRead))
		...
		AcDbObjectId entId;
		es = pBlkTblRcdItr->getEntityId(entId);
		
		if (!entId.objectClass()->isDerivedFrom(AcDbBlockReference::desc())) 
			continue;
...
int CNSDWGMgr::selectTextLabelsOnDP(std::map<NSSTRING, TEXTINFO_VECTOR*> &mapTextLabels, std::vector<NSSTRING> szCadastreLayers)
{
	...
	/*	if(Acad::eOk != pBlkTblRcdItr->getEntity(pEnt,AcDb::kForRead))
		...
		if (!entId.objectClass()->isDerivedFrom(AcDbText::desc())) 
			continue;
		es=acdbOpenObject(pEnt, entId, AcDb::kForRead);
...
int CNSDWGMgr::getArcSegments(AcDbEntity *pArcEntity, CNSCompositeGeometry *pTempPolyLine, CNSFeatureContainer *pFeatContainer)
{
	...
	pArc->getPointAtDist(pArc->length()/2, intermediatePoint);
...
int CNSDWGMgr::getArcSegmentsEx(AcDbEntity *pArcEntity, CNSCompositeGeometry *pTempPolyLine,  double sx, double sy, double sz,double ParentRotRad, double ParPosX, double ParPosY, CNSFeatureContainer *pSubFeatContainer)
{
	...
	//strArcSegment.append(szVertex);
	NSSTRING strArcSegment(&szVertex[0]);
...
//Added on 6th July 2019 for Bulk drawing issue
int CNSDWGMgr::getSTDLayerProperties(std::map<NSSTRING, CNSLayer*> &layerProperties)
{
	TCHAR* pszLayerName;
	TCHAR* pszLineTypeName;
	TCHAR* pszPlotStyleName;
	...
	AcDbLayerTable* pLayer;
	if (m_pStdDWGDatabase->getSymbolTable(pLayer, AcDb::kForRead) != Acad::eOk)
	{
		return NS_FAIL;
	}
...
int CNSDWGMgr::getTextStyleProperties(std::map<NSSTRING,CNSTextStyle*> &TextStyleProperties)
{
	...
	for(pTextIterator->start(); !pTextIterator->done(); pTextIterator->step())
	{
		...
		if(Acad::eOk != pTextStyleRecord->font(pFontName, bBold, bItalic, nCharSet, pitch,family))
		{
			pTextStyleRecord->close();
			pTextStyleTbl->close();
			return NS_FAIL;
...
//Added on 6th July 2019 for Bulk drawing issue
int CNSDWGMgr::getSTDTextStyleProperties(std::map<NSSTRING, CNSTextStyle*> &TextStyleProperties)
{
	Acad::ErrorStatus es = Acad::eOk;
	AcDbTextStyleTable *pTextStyleTbl = NULL;
	if (Acad::eOk != m_pStdDWGDatabase->getSymbolTable(pTextStyleTbl, AcDb::kForRead)) //get line table
	{
		return NS_FAIL;
	}
...
//Added on 6th July 2019 for Bulk drawing issue
int CNSDWGMgr::getSTDLineTypeList(std::map<NSSTRING, CNSLineType*> &LineTypeProperties)
{
	AcDbLinetypeTable *pLinetypeTbl = NULL;
	if (Acad::eOk != m_pStdDWGDatabase->getSymbolTable(pLinetypeTbl, AcDb::kForRead)) //get line table
		return NS_FAIL;
		...
		if(Acad::eOk == pDimStyleIter->getRecord(pDimStyleTableRecord, AcDb::kForRead))
		{
			...
			{	
				...
				if(Acad::eOk == acdbOpenObject(pObjTextStyle,nTxtStyleId,AcDb::kForRead))
				{
					...
					Autodesk::AutoCAD::PAL::FontUtils::FontFamily family = Autodesk::AutoCAD::PAL::FontUtils::FontFamily::kDoNotCare;
					Autodesk::AutoCAD::PAL::FontUtils::FontPitch pitch = Autodesk::AutoCAD::PAL::FontUtils::FontPitch::kDefault;
					if(Acad::eOk != pTextStyle->font(pszFontName, bbold, bItalic,pCharSet,pitch,family))
					{
						pTextStyle->close();
						pDimStyleTableRecord->close();
						pDimStyleTable->close();
...
//Added on 6th July 2019 for Bulk drawing issue
int CNSDWGMgr::getSTDDimStyleList(std::map<NSSTRING, CNSDimStyle*> &DimStyleProperties)
{
	AcDbDimStyleTable* pDimStyleTable = NULL;
	Acad::ErrorStatus es = Acad::eOk;

	//Purge unused _EXT layers
	if (Acad::eOk != m_pStdDWGDatabase->getSymbolTable(pDimStyleTable, AcDb::kForRead))
	{
		return NS_FAIL;
	}
	AcDbDimStyleTableIterator* pDimStyleIter;
...
//Added on 6th July 2019 for Bulk drawing issue
int CNSDWGMgr::checkSTDBlockAttributeVisibility(std::map<NSSTRING, std::map<NSSTRING, CNSAttributeRef*>*>&mapBlockAttrVisibility)
{
	AcDbBlockTable* pBlockTable = NULL;
	TCHAR* pszBlockName;

	m_pStdDWGDatabase->getBlockTable(pBlockTable, AcDb::kForRead);
	AcDbBlockTableIterator* pBlockTableIter;
	pBlockTable->newIterator(pBlockTableIter);
	AcDbBlockTableRecord* pBlockDef = NULL;
	CString strtocmp = _T("_USER");
...
*/
int CNSDWGMgr::getFeaturesToBeValidated(std::map<LONG_PTR, CNSValidationDataCollector*> &mapBlocksToBeValidated, std::map<LONG_PTR, CNSValidationDataCollector*> &mapLinearFeaturesToBeValidated, bool bGetXInfo, bool getLinerFeatures, TCHAR* pszLayerNameSuffix)
{
	...
	/*	if(Acad::eOk != pBlkTblRcdItr->getEntity(pEnt,AcDb::kForRead))
		{
			...
			AcDbObjectId entId;
		Acad::ErrorStatus es = pBlkTblRcdItr->getEntityId(entId);
		 //AcRxClass* pEntClass = AcDbCircle::desc();
			...
			&& (NSSTRCMP(strLayerNameSuffix.c_str(), _T("_EXT")) == 0))
		{
			...
			else			// it is a cable
			{
				...
				if(pEnt->isKindOf(AcDbPolyline::desc()))
				{
					...
					pCNSLinearFeature->setObjectId((LONG_PTR) pEnt->id().asOldId());
				...
				mapLinearFeaturesToBeValidated.insert(std::map<LONG_PTR, CNSValidationDataCollector*>::value_type(LONG_PTR(pEnt->id().asOldId()), pCNSLinearFeature));
			}
			pEnt->close();
			continue;
		}
		...
		pCNSValidationDataCollector->setObjectId((LONG_PTR) pEnt->id().asOldId());
		...
		mapBlocksToBeValidated.insert(std::map<LONG_PTR, CNSValidationDataCollector*>::value_type(pEnt->id().asOldId(), pCNSValidationDataCollector));
...
int CNSDWGMgr::getFeaturesOnLayer(std::map<LONG_PTR, CNSValidationDataCollector*> &mapBlocksToBeValidated, std::map<LONG_PTR, CNSValidationDataCollector*> &mapLinearFeaturesToBeValidated, int nRelationshipFlag, TCHAR* pszLayerName)
{
	AcDbBlockTable* pBlockTable = NULL;
	if (Acad::eOk != m_pCurrentDatabase->getBlockTable(pBlockTable, AcDb::kForRead))
		return NS_FAIL;
	...
	for (pBlkTblRcdItr->start(); !pBlkTblRcdItr->done(); pBlkTblRcdItr->step())
	{
		...
		if(!entId.objectClass()->isDerivedFrom(AcDbBlockReference::desc()) 
			...
			&& !entId.objectClass()->isDerivedFrom(AcDbLine::desc()) 
			&& !entId.objectClass()->isDerivedFrom(AcDbArc::desc())
			&& !entId.objectClass()->isDerivedFrom(AcDbMline::desc()) // Change for BUSBAR implementation
			)
		{
			continue;
		}
		...
		else if(pEnt->isKindOf(AcDbMline::desc())) 
		{
			...
			mapLinearFeaturesToBeValidated.insert(std::map<LONG_PTR, CNSValidationDataCollector*>::value_type(LONG_PTR(pEnt->id().asOldId()), pCNSValidationDataCollector));
			pEnt->close();
			continue;	
		...
		//		// Add the CNSValidationDataCollector to the map to be returned
		mapBlocksToBeValidated.insert(std::map<LONG_PTR, CNSValidationDataCollector*>::value_type(LONG_PTR(pEnt->id().asOldId()), pCNSValidationDataCollector));
...
int CNSDWGMgr::getTextHeight(std::vector<CNSTextHeight*> &vectTxtHt)
{
	...
	for (pBlkTblRcdItr->start(); !pBlkTblRcdItr->done(); pBlkTblRcdItr->step())
	{
		...
		AcDbObjectId entId;
		es = pBlkTblRcdItr->getEntityId(entId);
		 AcRxClass* pEntClass = AcDbText::desc();
		//es = pBlkTblRcdItr->getEntity(pEnt,AcDb::kForWrite);
		if(es!=eOk)
		{
			continue;
		}
		if (entId.objectClass()->isDerivedFrom(pEntClass)) 
		{
...
*/
int CNSDWGMgr::getPlineRecord(LONG_PTR ObjectId, CNSPoint3D &startPoint, CNSPoint3D &nextPoint, double &dArea)
{
...
int CNSDWGMgr::selectNonStdBlocks(std::map<LONG_PTR, CNSValidationDataCollector*> &mapBlocksToBeValidated)
{
	Acad::ErrorStatus es = Acad::eOk;
...
*/
		...
		AcDbObjectId entId;
		es = pBlkTblRcdItr->getEntityId(entId);
		 AcRxClass* pEntClass = AcDbBlockReference::desc();
...
int CNSDWGMgr::GetInsertionPoint(LONG_PTR nObjectID, CNSPoint3D &pt)
{
	Acad::ErrorStatus es;
	AcDbObject *pEntity;
	AcDbObjectId pBlockObjID;
	pBlockObjID.setFromOldId((Adesk::UIntPtr)nObjectID);
...
//}
	...
	{		
		...
		AcDbObjectId entId;
		Acad::ErrorStatus es = pBlkTblRcdItr->getEntityId(entId);
		 AcRxClass* pEntClass = AcDbLine::desc();
		//es = pBlkTblRcdItr->getEntity(pEnt,AcDb::kForWrite);
		if(es!=eOk)
		{
			continue;
				...
				for(int i=0;i<ids.length();i++)
				{
					...
					if (Acad::eOk == acdbOpenObject(pObj, id, AcDb::kForRead))
					{
						...
						//Get all Attributes
							...
							try
							{
								if (pBTRItr == NULL) continue;
								//AcDbObjectIdArray idsEntArray;
								for (pBTRItr->start(); !pBTRItr->done(); pBTRItr->step())
							{	
								//Added on 22nd June 2019 for the crash in Hatch in Client's Logo
								/*if (pBTRItr == NULL) continue;
								AcDbObjectId idEntity;
								if (pBTRItr->getEntityId(idEntity) != Acad::eOk) continue;
								idsEntArray.append(idEntity);*/

								//Commented on 22nd June 2019 for the crash in Hatch in Client's Logo
								AcDbEntity* pAttDefEnt = NULL;
									if (Acad::eOk == pBTRItr->getEntity(pAttDefEnt, AcDb::kForRead,true))
									{
										//nCount = nCount + 1;
										if (pAttDefEnt == NULL)
										{
								...
								/*for (int n = 0; n < idsEntArray.length()-3; n++)
								{
									try
									{
										AcDbObjectPointer<AcDbAttributeDefinition>pTempAttdef(idsEntArray.at(n), AcDb::kForRead);
										if (pTempAttdef.openStatus() != Acad::eOk)
										{
											continue;
										}
										TCHAR* pszAttDefTag = pTempAttdef->tag();
								...
								for (int j = idsEntArray.length()-1 ; j > idsEntArray.length() - 2; j--)
								{
									AcDbObjectPointer<AcDbAttributeDefinition>pAttDef(idsEntArray.at(j), AcDb::kForRead);
									if (pAttDef.openStatus() != Acad::eOk)
									{
										continue;
									}
									TCHAR* pszAttDefTag = pAttDef->tag();
									STRING_MAP::iterator itrMapofEmptyAttr = mapOfEmptyAttributes.find(pszAttDefTag);
									if (itrMapofEmptyAttr == mapOfEmptyAttributes.end())
									{
										pAttDef->close();
									...
									TCHAR* pszAttDefPrompt = pAttDef->prompt();
									NSSTRING strAttDefPrompt = (NSSTRING)pszAttDefPrompt;
									int nDot = strAttDefPrompt.find_last_of(_T("("));
									strAttDefPrompt = strAttDefPrompt.substr(nDot + 1, (strAttDefPrompt.length() - 2));
									if (NSSTRCMP(strAttDefPrompt.c_str(), _T("GIS)")) == 0)
...
*/
int CNSDWGMgr::getPromptValues(LONG_PTR lobjID, std::map<NSSTRING, NSSTRING> &mapofPrompValues)
{
	Acad::ErrorStatus es;
	AcDbObjectId objID;
	objID.setFromOldId((Adesk::UIntPtr)lobjID);
	if (!objID.isValid())
	{
		return NS_FAIL;
	}
...
NSSTRING CNSDWGMgr::getVersionInfo(CNSDWGMgr &pStdDatabase)
{
	...
	AcDbDatabase* srcDb = pStdDatabase.m_pStdDWGDatabase;
...
int CNSDWGMgr::scanAssets(ASSET_VECTOR &m_vectofAssets)
{
	...
	for (pBlkTblRcdItr->start(); !pBlkTblRcdItr->done(); pBlkTblRcdItr->step())
	{
		...
		if(pEnt->isKindOf(AcDbBlockReference::desc()))
		{
			...
			if(rFindString(strEntityLayer,strLayerNameSuffix)==-1)
			{
				pEnt->close();
				continue;
			}
		...
		for(attributeIterator->start(); !attributeIterator->done(); attributeIterator->step())
		{
			...
			if(NSSTRCMP(pszKey, _T("ASSET_NUM")) == 0)
			{
				...
				pCNSAsset->setObjectID((LONG_PTR) pEnt->id().asOldId());
...
int CNSDWGMgr::getAssetNumbers(std::vector<CNSAsset*> & assetVector)
{
	...
	for (pBlkTblRcdItr->start(); !pBlkTblRcdItr->done(); pBlkTblRcdItr->step())
	{
		...
		//if(bIsNew == false)
		if(rFindString(strEntityLayer, _T("_NEW")) == -1)
		{
			pEnt->close();
			continue;
		}
	...
	else
	{
		...
		for (; !pBlockIterator->done(); pBlockIterator->step())
		{
			...
			{	
				...
				double ActualChildRotDeg = ActualChildRotRad*(180.0/PI);
...
int CNSDWGMgr::getBlocksOnNonMatchingLayer(std::map<LONG_PTR, CNSValidationDataCollector*> &mapBlocksOnWrongLayer,   TCHAR* pszNameSuff)
{
	...
	for (pBlkTblRcdItr->start(); !pBlkTblRcdItr->done(); pBlkTblRcdItr->step())
	{
		...
		if (NSSTRCMP(strBlockNameSuffix.c_str(),pszNameSuff) == 0) // dont search for other layer
		{
			...
			if (NSSTRCMP(strLayerNameSuffix.c_str(),pszNameSuff) != 0) 
			{
				...
				mapBlocksOnWrongLayer.insert(std::map<LONG_PTR, CNSValidationDataCollector*>::value_type(LONG_PTR(pEnt->id().asOldId()), pCNSValidationDataCollector));
...
{	
	...
	if (txtStr == L"AC1032")
		dwgverstr = L"AutoCAD 2018";
	else if(txtStr==L"AC1027")
		dwgverstr=L"AutoCAD 2013";
	else if(txtStr==L"AC1024")

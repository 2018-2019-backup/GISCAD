...
{
	HMODULE hModule = GetModuleHandle(L"RealDWGProcessor.dll");

	if (hModule != NULL)
	{
		typedef void(*LPGETNUMBER)(void);
		LPGETNUMBER lpReleaseRealDWG = (LPGETNUMBER)GetProcAddress((HMODULE)hModule, "ReleaseRealDWG");

		if (lpReleaseRealDWG != NULL)
		{
			lpReleaseRealDWG();
		}
	}

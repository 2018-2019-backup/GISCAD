...
BEGIN_MESSAGE_MAP(CNSHyperEdit, CEdit)
	//{{AFX_MSG_MAP(CNSHyperEdit)
...
void CNSHyperEdit::PreSubclassWindow() 
{
...
void CNSHyperEdit::OnMouseMove(UINT nFlags, CPoint point) 
{
...
void CNSHyperEdit::OnLButtonDown(UINT nFlags, CPoint point) 
{
...
void CNSHyperEdit::OnLButtonUp(UINT nFlags, CPoint point) 
{
	...
	CString csURL = IsHyperlink(point);
/*	CString csCommandParam = _T("\"") + csURL;
	csCommandParam += _T("\"")*/;
...
LRESULT CNSHyperEdit::WindowProc(UINT message, WPARAM wParam, LPARAM lParam) 
{
...
void CNSHyperEdit::OnTimer(UINT_PTR nIDEvent) 
{
	//

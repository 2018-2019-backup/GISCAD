...
int CNSRelationLinker::initializeFeatures(bool bProposedToExisting, int LinearConnOnLayerFlag)
{
	...
	m_pLocalDWGMgr->getFeaturesToBeValidated(m_mapBlocksToBeLinked, m_mapLinearFeaturesToBeLinked, true, true, _T("_NEW"));//,tempReferenceStorage);

	TCHAR temp[MAX_PATH+1];
	//NSSPRINTF(temp,_T("Blocks = %d Linear = %d"), m_mapBlocksToBeLinked.size(), m_mapLinearFeaturesToBeLinked.size());

	//MessageBox(NULL, temp, _T("Test"), MB_OK);

	// Do not take cables on MODlayers
	m_pLocalDWGMgr->getFeaturesToBeValidated(m_mapBlocksToBeLinked, m_mapLinearFeaturesToBeLinked, true, false, _T("_MOD"));//,tempReferenceStorage);
	if(bProposedToExisting)
	{
		m_pLocalDWGMgr->getFeaturesToBeValidated(m_mapExtBlocks, m_mapExtCablesToBeLinked, true, true, _T("_EXT"));//,tempReferenceStorage);
	}
...
int CNSRelationLinker::getOwnershipParentsFromMDB(CNSValidationDataCollector* pFeature, std::vector<int> &vectOwnershipParents)
{
	...
	{
		/*NSSPRINTF(szQuery, _T("Result = %d"), (int)(*varientVectItr));
		MessageBox(NULL, szQuery, _T("Test"), MB_OK);*/
...
int CNSRelationLinker::getManyOwnershipChilds(CNSValidationDataCollector* pFeature, std::vector<int> &vectManyOwnershipChilds)
{
	...
	//MessageBox(NULL, szQuery1 , _T("Test"), MB_OK);
	//NSSPRINTF(szQuery1,_T("Count = %d"), vectManyOwnershipChilds.size());
	//MessageBox(NULL, szQuery1 , _T("Test"), MB_OK);
...
//NSSPRINTF(szQuery2,_T("Count = %d"), vectManyOwnershipChilds.size());
//	MessageBox(NULL, szQuery2 , _T("Test"), MB_OK);
...
void CNSRelationLinker::assignOwnershipParentChild(CNSValidationDataCollector* pFeature)
{
	...
	if(nParentFID != nNumberNotFound)
	{
		...
		//18-11-2014 - KKM
		if(pParentFeature->getBlockName() == _T("PILLAR_NEW") || pParentFeature->getBlockName() == _T("PANEL_NEW"))
		{
			if(pFeature->getBlockName() == _T("PILLAR_NEW") || pFeature->getBlockName() == _T("PANEL_NEW"))
			{

				NSSTRING strAttribute =_T("PARENT");
				NSSTRUPR((TCHAR*)strAttribute.c_str(), strAttribute.size()+1);
				std::map<NSSTRING, NSSTRING>::iterator itr_feature;
				std::map<NSSTRING, NSSTRING>::iterator itr_Parent;
				itr_feature = pFeature->m_mapAttributes.find((TCHAR*)strAttribute.c_str());
				itr_Parent = pParentFeature->m_mapAttributes.find((TCHAR*)strAttribute.c_str());


				if(itr_feature != pFeature->m_mapAttributes.end() && itr_Parent != pParentFeature->m_mapAttributes.end())
				{
					NSSTRING strFeatureValue = (*itr_feature).second;
					NSSTRING strParentValue = (*itr_Parent).second;
					if(NSSTRCMP(strFeatureValue.c_str(), _T("")) == 0 || NSSTRCMP(strParentValue.c_str(), _T("")) == 0)
					{
						//		NSPRINT(_T("Mandatory Information not available for asset"));
						//MessageBox(NULL, _T("1"), _T("Test"), MB_OK);
						return;
					}
					else if(NSSTRCMP(strFeatureValue.c_str(),strParentValue.c_str()) !=0)
					{
					...
					{
						//MessageBox(NULL, _T("3"), _T("Test"), MB_OK);
						//Proceed
					}
				}
				else
...
void CNSRelationLinker::postasXDATA()
{
	...
	//TCHAR temp[1000];
	//	NSSPRINTF(temp,_T("Proposed = %d"), m_mapProposedRelations.size());
	...
	//MessageBox(NULL, strFID.c_str(), _T("Test"), MB_OK);
		...
		//MessageBox(NULL, _T("xdata") , _T("Test"), MB_OK);
		m_pLocalDWGMgr->addXDATA(lObjectId, mapXDATA);//, tempReferenceStorage);

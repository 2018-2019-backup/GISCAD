...
class _declspec(dllexport)CNSDWGMgr
{
public:
	CNSDWGMgr(void);
	...
	int saveCurrentDWG(TCHAR* pszFileName, bool bAs2004, NSSTRING saveFormat = _T("AutoCAD 2010"));
	...
	int getFeaturesToBeValidated(std::map<LONG_PTR, CNSValidationDataCollector*> &mapBlocksToBeValidated, std::map<LONG_PTR, CNSValidationDataCollector*> &mapLinearFeaturesToBeValidated, bool bGetXInfo, bool getLinerFeatures, TCHAR* pszLayerNameSuffix);//, std::map<long, AcDbObjectId> &tempReferenceStorage);
...
//	int scanSubStationAssets(std::map<long, SWITCH_MAP*> & mapofSwitches);
	...
	int setAssetNumber(LONG_PTR blkObjID, NSSTRING strAssetNo);
	int readBlockAttributes(LONG_PTR nBlockObjID, std::map<NSSTRING, NSSTRING> & mapOfAttributes);
	int getBlockName(LONG_PTR nBlockObjID, NSSTRING &strBlockName);

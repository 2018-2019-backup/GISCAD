...
void CNSAsset::setObjectID(const LONG_PTR nObjID)
{
	m_nObjectID = nObjID;
}

LONG_PTR CNSAsset::getObjectID() const
{
	return m_nObjectID;
}
...
int CNSAsset::checkAttribute(NSSTRING strAttribute, std::map<NSSTRING, NSSTRING> & mapOfAttributes)
{
	...
	itr = mapOfAttributes.find(strupr((TCHAR*)strAttribute.c_str()));
	if(itr != mapOfAttributes.end())
	{
...
NSSTRING CNSAsset::getEquipNumberD1()
{
	...
	std::map<NSSTRING, NSSTRING> blockAttributes;
	readBlockAttributes(blockAttributes);
	std::map<NSSTRING, NSSTRING>::iterator it;
	it = blockAttributes.find(_T("SUBURB"));
	if(it != blockAttributes.end())
	{
		strSubUrb = (*it).second;
	}
	it = blockAttributes.find(_T("STREETNAME"));
	if(it != blockAttributes.end())
	{
		strStreetName = (*it).second;
	}
	NSSTRING strStreet = strStreetName.append("-");

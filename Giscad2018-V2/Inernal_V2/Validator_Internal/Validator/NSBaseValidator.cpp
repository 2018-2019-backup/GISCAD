...
//	_wasctime_s(szFormat, _MAX_PATH, &time);
	...
	if( !GetUserName(szUserName, &dwBufferSize))
	{
		...
		//-----------NSSTRCPY(szUserName, titleblockTbl.getUserName().size() + 1, (TCHAR*)titleblockTbl.getUserName().c_str());
	}
	...
	while (szUserName[i])
	{
		if ((szUserName[i] < 65 || (szUserName[i] > 90 && szUserName[i] < 97) || szUserName[i] > 122) && szUserName[i] != ' ')
		{
			szUserName[i] = ' ';
		}
		if (isalpha(szUserName[i]))
		{
		}
		i++;
	}*/
	/*for (int j = 0; j <= dwBufferSize; j++)
	{
		if (!isalpha(szUserName[j]))
		{
			szUserName[j] = ' ';
		}
	}*/

...
CNSTabCtrlSSL::~CNSTabCtrlSSL (void) {
}

BEGIN_MESSAGE_MAP(CNSTabCtrlSSL, CTabCtrl)
...
int CNSTabCtrlSSL::AddSSLPage (LPCTSTR pszTitle, int nPageID, CTabPageSSL* pTabPage) {
...
int CNSTabCtrlSSL::AddSSLPage (LPCTSTR pszTitle, int nPageID, LPCTSTR pszTemplateName) {
	...
	if (!pDialog->Create (pszTemplateName, GetParent())) {
...
int CNSTabCtrlSSL::AddSSLPage (LPCTSTR pszTitle, int nPageID, int nTemplateID) {
	return AddSSLPage (pszTitle, nPageID, MAKEINTRESOURCE (nTemplateID));
}

BOOL CNSTabCtrlSSL::RemoveSSLPage (int nIndex) {
...
int CNSTabCtrlSSL::GetSSLPageCount (void) {
	return GetItemCount ();
}

BOOL CNSTabCtrlSSL::GetSSLPageTitle (int nIndex, CString &strTitle) {
	if (nIndex >= GetItemCount ())
		return FALSE;
...
BOOL CNSTabCtrlSSL::SetSSLPageTitle (int nIndex, LPCTSTR pszTitle) {
	if (nIndex >= GetItemCount ())
		return FALSE;
...
int CNSTabCtrlSSL::GetSSLPageID (int nIndex) {
	if (nIndex >= GetItemCount ())
		return -1;
...
int CNSTabCtrlSSL::SetSSLPageID (int nIndex, int nPageID) {
	if (nIndex >= GetItemCount ())
		return -1;
...
BOOL CNSTabCtrlSSL::ActivateSSLPage (int nIndex) {
	if (nIndex >= GetItemCount ())
		return FALSE;
...
int CNSTabCtrlSSL::GetSSLActivePage (void) {
	return GetCurSel ();
}

CWnd* CNSTabCtrlSSL::GetSSLPage (int nIndex) {
	if (nIndex >= GetItemCount ())
		return NULL;
...
int CNSTabCtrlSSL::GetSSLPageIndex (int nPageID) {
	int nCount = GetItemCount ();
	if (nCount == 0)
		return -1;
...
#ifdef _DEBUG
BOOL CNSTabCtrlSSL::CheckDialogTemplate (LPCTSTR pszTemplateName) {
...
void CNSTabCtrlSSL::ResizeDialog (int nIndex, int cx, int cy) {
	if (nIndex != -1) {
        TabDelete tabDelete = m_tabs[nIndex];
        CTabPageSSL* pDialog = tabDelete.pTabPage;
		...
		if (pDialog != NULL) {
			...
			if (dwStyle & TCS_VERTICAL) { // Vertical tabs
				...
				x = rectWnd.left+2;
				y = rectWnd.top+((dwStyle & TCS_BOTTOM) ? 4 : nTabHeight + 4);
				nWidth = rectWnd.right-rectWnd.left-4;
				nHeight = rectWnd.bottom-rectWnd.top-nTabHeight-8;
				}
...
BOOL CNSTabCtrlSSL::OnInitPage (int nIndex, int nPageID) {
	// TODO: Override in derived class to initialise pages.
	return TRUE;
}

void CNSTabCtrlSSL::OnActivatePage (int nIndex, int nPageID) {
	// TODO: Override in derived class to respond to page activations.
}

void CNSTabCtrlSSL::OnDeactivatePage (int nIndex, int nPageID) {
	// TODO: Override in derived class to respond to page deactivations.
}

void CNSTabCtrlSSL::OnDestroyPage (int nIndex, int nPageID) {
...
void CNSTabCtrlSSL::OnSelChanging (NMHDR* pNMHDR, LRESULT* pResult) {
...
void CNSTabCtrlSSL::OnSelChange (NMHDR* pNMHDR, LRESULT* pResult) {
	int nIndex = GetCurSel ();
	if (nIndex == -1)
		return;
...
/*
void CNSTabCtrlSSL::OnSetFocus (CWnd* pOldWnd) {
...
void CNSTabCtrlSSL::OnKillFocus (CWnd* pNewWnd) {
...
void CNSTabCtrlSSL::OnDestroy (void) {
...
BOOL CNSTabCtrlSSL::OnCommand (WPARAM wParam, LPARAM lParam) {
	// Forward WM_COMMAND messages to the dialog's parent.
	return GetParent ()->SendMessage (WM_COMMAND, wParam, lParam);
}

BOOL CNSTabCtrlSSL::OnNotify (WPARAM wParam, LPARAM lParam, LRESULT* pResult) {
	// Forward WM_NOTIFY messages to the dialog's parent.
	return GetParent ()->SendMessage (WM_NOTIFY, wParam, lParam);
}

BOOL CNSTabCtrlSSL::OnCmdMsg (UINT nID, int nCode, void* pExtra,
...
int CNSTabCtrlSSL::AddPage (LPCTSTR pszTitle, int nPageID, TabDelete tabDelete) {
	// Add a page to the tab control.
	TC_ITEM item;
	item.mask = TCIF_TEXT;
	item.pszText = (LPTSTR) pszTitle;
    ...
    if (NULL == tabDelete.pTabPage) {
		...
		INT_PTR nArrayIndex = m_tabs.Add (tabDelete);
        ...
        tabDelete.pTabPage->SetParent (GetParent()); // Just to be sure

...
void CNSLoaderDlg::OnBnClickedBtnConvert()
{
	...
	for(int nFileno = 0; nFileno < nCount; nFileno++)
	{
		...
		int nretValue = isDirectoryExists(strOutputFile1.GetBuffer());
		strOutputFile1.ReleaseBuffer();
		...
		//Get the command used for conversion in a string
		...
		TCHAR* pszCmd = strPath.GetBuffer();
		strPath.ReleaseBuffer();
		BeginWaitCursor();
		
		nRetVal=(int)ExecuteConverterApp(pszCmd);
		Invalidate();
        EndWaitCursor();
        m_ProgressBar.SetPos(nProgressCount++);
		...
		//pValidator->setDWGFilePath(strDwgPath.GetBuffer(0));
		//pValidator->setStdDWGFilePath(L"");
		//pValidator->ReadDwgHeader();
...
void CNSLoaderDlg::OnBnClickedBtnValidate()
{
	...
	if(NSACCESS(strFilename.c_str(), 0) != -1)
	{
		...
		for(int nItemCntr = 0; nItemCntr < nCount; nItemCntr++)
		{
			...
			pValidator->setDWGFilePath(szDwgPath.GetBuffer());
			szDwgPath.ReleaseBuffer();
            ...
            if(m_ChkAssetNumber.GetCheck()==1)
	        {
				...
				NSSTRCPY(userCredentials.m_szUserName, strUserName.GetBuffer()); strUserName.ReleaseBuffer();
		        NSSTRCPY(userCredentials.m_szPassword, strPwd.GetBuffer()); strPwd.ReleaseBuffer();
		        NSSTRCPY(userCredentials.m_szDistrict, strDistrict.GetBuffer()); strDistrict.ReleaseBuffer();
		        NSSTRCPY(userCredentials.m_szPosition, strPosition.GetBuffer()); strPosition.ReleaseBuffer();
                NSSTRING strKey(strHostID.GetBuffer()); strHostID.ReleaseBuffer();
			...
			NSSTRING temp = pValidator->report;

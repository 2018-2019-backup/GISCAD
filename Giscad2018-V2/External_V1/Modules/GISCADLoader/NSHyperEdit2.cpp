...
void CNSHyperEdit::DrawHyperlinks()
{
	CRect rcRect;
		...
		for(int j=m_linkOffsets[i].iStart; j<(m_linkOffsets[i].iStart+m_linkOffsets[i].iLength); j++){
			...
			if(IsSelection(iSelStart, iSelFinish)){
				...
				if(j>=iSelStart && j<iSelFinish)
					...
					pDC->TextOut(pt.x, pt.y, szTemp); // Draw overtop of existing character
			}
			else // No selection, just draw normally
				pDC->TextOut(pt.x, pt.y, szTemp); 
...
void CNSHyperEdit::BuildOffsetList(int iCharStart, int iCharFinish)
{
...
CString CNSHyperEdit::IsHyperlink(CPoint& pt) const 
{
...
BOOL CNSHyperEdit::IsWhiteSpace(const CString& csBuff, int iIndex) const 
{ 
	// Check for End of buffer 
	if(csBuff.GetLength() == iIndex) return TRUE;
		
	// Check for whitespace
	//if(csBuff.GetAt(iIndex) == WHITE_SPACE1) 
        //return TRUE;
	if(csBuff.GetAt(iIndex) == WHITE_SPACE2) 
        return TRUE;
	if(csBuff.GetAt(iIndex) == WHITE_SPACE3) 
        return TRUE;
	if(csBuff.GetAt(iIndex) == WHITE_SPACE4) 
        return TRUE;
...
BOOL CNSHyperEdit::IsWordHyper(const CString& csToken) const
{

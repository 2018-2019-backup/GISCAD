...
bool CNSXMLParser::loadFile(NSSTRING strFileName)
{
	try
	{
		...
		if (FAILED(hr)) 
		{
			...
			MessageBox(NULL, errMsg, _T("COM Error"), MB_OK);
	...
	catch (_com_error &error)
	{
		printf("Error\n");
		printf("\a\tCode = %08lx\n", error.Error());
		printf("\a\tCode meaning = %S", error.ErrorMessage());
		_bstr_t bstrSource(error.Source());
		_bstr_t bstrDescription(error.Description());
		printf("\a\tSource = %s\n", (LPCSTR)bstrSource);
		printf("\a\tDescription = %s\n", (LPCSTR)bstrDescription);
		MessageBox(NULL, error.ErrorMessage(), _T("COM Error"), MB_OK);
		return false;
	}
	catch(...)
	{
...
bool CNSXMLParser::validate(const NSSTRING strFileName)
{
	...
	try
	{
		HRESULT hr = pXSDDoc.CreateInstance(__uuidof(DOMDocument60));
		if (FAILED(hr))
		{
			WriteToLog(LOG_OTHER, _T("Cann't create XMLDocument"));
			return false;
	...
	if (pError->errorCode != 0)
	{
		...
		MessageBox(NULL, bstrErr, _T("COM Error"), MB_OK);

...
#include "dbxrecrd.h"
#include "sstream"
#import "AcStMgr.tlb" no_namespace raw_interfaces_only named_guids
...
int CNSDWGMgr::openDWG(TCHAR* p_szFileName, bool bOpenExisting)
{
	...
	if(m_pCurrentDatabase != NULL)
	{
		...
		{
			es = m_pCurrentDatabase->readDwgFile(p_szFileName, AcDbDatabase::OpenMode::kForReadAndAllShare);
...
int CNSDWGMgr::saveCurrentDWG(TCHAR* p_szFileName, bool bAs2004, NSSTRING saveFormat)
{
	Acad::ErrorStatus st = Acad::eOk;
		...
		if(m_pCurrentDatabase != NULL)
		{
			...
			{
				st = m_pCurrentDatabase->saveAs(p_szFileName);
			}
		}
	...
	for(; !pDicIter->done(); pDicIter->next())
	{
		...
		for (pBlockIterator->start(); !pBlockIterator->done(); pBlockIterator->step())
		{
			...
			for(pBTRItr->start(); !pBTRItr->done(); pBTRItr->step())
			{
				...
				pBTRItr->getEntity(pAttDefEnt, AcDb::kForRead);
...
int CNSDWGMgr::addXDATA(LONG_PTR lObjectId, std::map<NSSTRING, NSSTRING> &mapXData)
{
	...
	if(lObjectId<=0) 
	{
		...
		//MessageBox(NULL, _T("1"), _T("Test"), MB_OK);
		AcDbEntity *pEnt = NULL;
		for (pBlkTblRcdItr->start(); !pBlkTblRcdItr->done(); pBlkTblRcdItr->step())
		{
			if(Acad::eOk != pBlkTblRcdItr->getEntity(pEnt,AcDb::kForWrite))
			{
				/*pRecord->close();
				pBlockTable->close();
				value = 0;*/
			...
			{
				if(((LONG_PTR)(pEnt->id().asOldId())) == lObjectId)
				{				
					//MessageBox(NULL, _T("Got ID"), _T("Test"), MB_OK);
					std::map<NSSTRING, NSSTRING>::iterator mapItr;
					for(mapItr = mapXData.begin(); mapItr != mapXData.end(); mapItr++)
					{
						NSSTRING strAppName, strAppValue;
						...
						//MessageBox(NULL, strAppName.c_str(), _T("Test"), MB_OK);

						struct  resbuf  *pRb, *pTemp;    
						pRb = pEnt->xData(strAppName.c_str());
						...
						{	
							//MessageBox(NULL, _T("2"), _T("Test"), MB_OK);
						...
						{
							//MessageBox(NULL, strAppValue.c_str(), _T("Test"), MB_OK);
						}
					...
					//MessageBox(NULL, _T("Close Entity"), _T("Test"), MB_OK);					
		...
		if(value == 0)
		{
			//MessageBox(NULL, _T("Id Missed"), _T("Test"), MB_OK);

			ObjectId.setNull();
		}
		else
		{
			//MessageBox(NULL, _T("value is not 0"), _T("Test"), MB_OK);
		}

		//MessageBox(NULL, _T("Close Record"), _T("Test"), MB_OK);
		pRecord->close();
		//MessageBox(NULL, _T("Close Table"), _T("Test"), MB_OK);
		pBlockTable->close();
		//MessageBox(NULL, _T("Close Iterator"), _T("Test"), MB_OK);
	...
	{
		ObjectId.setFromOldId((LONG_PTR)&lObjectId);
	...
	AcDbObject *pObj;
	try{
		if (ObjectId.isValid() && ObjectId.isResident())
		{
			if (Acad::eOk != (es = acdbOpenAcDbObject(pObj, ObjectId, AcDb::kForWrite)))
			{
				//MessageBox(NULL, _T("Crash Failed"), _T("Test"), MB_OK);
				return NS_FAIL;
			}
		}
		else
	...
	{
		//MessageBox(NULL, _T("Failed"), _T("Test"), MB_OK);
		return -1;
	}
	std::map<NSSTRING, NSSTRING>::iterator mapItr;
	for(mapItr = mapXData.begin(); mapItr != mapXData.end(); mapItr++)
...
{
	if (!acrxLoadModule(_T("AcMPolygonObj20.dbx"), false))
		return NS_FAIL;
...
int CNSDWGMgr::getTextStyleProperties(std::map<NSSTRING,CNSTextStyle*> &TextStyleProperties)
{
	...
	for(pTextIterator->start(); !pTextIterator->done(); pTextIterator->step())
	{
		...
		Charset nCharSet;
		Autodesk::AutoCAD::PAL::FontUtils::FontPitch picth = Autodesk::AutoCAD::PAL::FontUtils::FontPitch::kDefault;
		Autodesk::AutoCAD::PAL::FontUtils::FontFamily family = Autodesk::AutoCAD::PAL::FontUtils::FontFamily::kDoNotCare;
		if(Acad::eOk != pTextStyleRecord->font(pFontName, bBold, bItalic, nCharSet, picth, family))
		{
			pTextStyleRecord->close();
			pTextStyleTbl->close();
			return NS_FAIL;
		...
		if(Acad::eOk == pDimStyleIter->getRecord(pDimStyleTableRecord, AcDb::kForRead))
		{
			...
			{	
				...
				if(Acad::eOk == acdbOpenObject(pObjTextStyle,nTxtStyleId,AcDb::kForRead))
				{
					...
					Autodesk::AutoCAD::PAL::FontUtils::FontPitch picth = Autodesk::AutoCAD::PAL::FontUtils::FontPitch::kDefault;
					Autodesk::AutoCAD::PAL::FontUtils::FontFamily family = Autodesk::AutoCAD::PAL::FontUtils::FontFamily::kDoNotCare;
					if(Acad::eOk != pTextStyle->font(pszFontName, bbold, bItalic,nCharSet,picth, family))
					{
						pTextStyle->close();
						pDimStyleTableRecord->close();
						pDimStyleTable->close();
...
int CNSDWGMgr::checkBlockAttributeVisibility(std::map<NSSTRING, std::map<NSSTRING,CNSAttributeRef*>*> &mapBlockAttrVisibility)
{
	...
	for(pBlockTableIter->start(); !pBlockTableIter->done(); pBlockTableIter->step())
	{
		...
		if(strTempBlkName.size() > 6)
		{
			NSSTRING strTempBlkNameSuffix = strTempBlkName.substr((strTempBlkName.size()-6), 6);
			if(NSSTRCMP(strTempBlkNameSuffix.c_str(), _T("_TITLE")) == 0 )
			{
				pBlockDef->close();
				continue;
			}
...
*/
int CNSDWGMgr::getFeaturesToBeValidated(std::map<LONG_PTR, CNSValidationDataCollector*> &mapBlocksToBeValidated, std::map<LONG_PTR, CNSValidationDataCollector*> &mapLinearFeaturesToBeValidated, bool bGetXInfo, bool getLinerFeatures, TCHAR* pszLayerNameSuffix)//,std::map<long, AcDbObjectId> &tempReferenceStorage)
{
	...
	for (pBlkTblRcdItr->start(); !pBlkTblRcdItr->done(); pBlkTblRcdItr->step())
	{
			...
			&& (NSSTRCMP(strLayerNameSuffix.c_str(), _T("_EXT")) == 0))
		{
			...
			else			// it is a cable
			{
				...
				//tempReferenceStorage.insert(std::map<long, AcDbObjectId>::value_type((long)pEnt->id().asOldId(), pEnt->id()));
			}
			pEnt->close();
			continue;
		}
			...
			) 
        {
			...
			{						
				CNSValidationDataCollector *pCNSLinearFeature = new CNSValidationDataCollector;
				pCNSLinearFeature->setObjectId(pEnt->id().asOldId());
//				long temp;
//NSSPRINTF(temp, _T("%l"), pEnt->id());
//				pCNSLinearFeature->setObjectId(temp);
				pCNSLinearFeature->setFeatureType(FEATURE_LINEAR);
		...
		//Commented for skipping suffix validation
		...
		if(findString(strEntityLayer.c_str(), strLayerNameSuffix.c_str()) == -1)
		{
			pEnt->close();
			continue;
		}
...
int CNSDWGMgr::getFeaturesOnLayer(std::map<LONG_PTR, CNSValidationDataCollector*> &mapBlocksToBeValidated, std::map<LONG_PTR, CNSValidationDataCollector*> &mapLinearFeaturesToBeValidated, int nRelationshipFlag, TCHAR* pszLayerName)
{
	...
	for (pBlkTblRcdItr->start(); !pBlkTblRcdItr->done(); pBlkTblRcdItr->step())
	{
		...
		//MessageBox(NULL, strEntityLayer.c_str(), _T("Test"),MB_ICONWARNING);
...
//		// Add the CNSValidationDataCollector to the map to be returned
		...
		//MessageBox(NULL,_T("1"), _T("Test"),MB_ICONWARNING);
...
//	IAcStError *pError;	
//
//	if(FAILED(hr*pComEnt->get_ObjectID32(&ObjectId)))
//return NS_FAIL;
...
int CNSDWGMgr::GetInsertionPoint(LONG_PTR nObjectID, CNSPoint3D &pt)
{
	...
	{
	pBlockObjID.setFromOldId((LONG_PTR)&nObjectID);
	}
...
int CNSDWGMgr::WriteToLoglayout(NSSTRING strLog)
{
	...
	if(lytId != NULL)
	{
		...
		Adesk::Int32  SelectionSetLength = 0;
		NSSTRING strMsg;
...
////////////////////////
			...
			for(pIter->start(); !pIter->done(); pIter->step())
			{
				...
				std::map<NSSTRING, NSSTRING>::iterator itrMap;
				//itrMap = mapTitleAttributes.find(szAttName);
				// compare the values of the attribute and if different store it in a map.
			/*	if(itrMap != mapTitleAttributes.end())
				{ 
					if((*itrMap).second != szAttValue)
					{
...
int CNSDWGMgr::getPromptValues(LONG_PTR lobjID, std::map<NSSTRING, NSSTRING> &mapofPrompValues)
{
	Acad::ErrorStatus es;
	AcDbObjectId objID;
	if(lobjID<=(LONG_PTR)0)
	{
		objID.setNull();
	}
	else
	{
	objID.setFromOldId((LONG_PTR)&lobjID);
	}
...
int CNSDWGMgr::setAssetNumber(LONG_PTR blkObjID, NSSTRING strAssetNo)
{
	...
	{
	pBlockObjID.setFromOldId((LONG_PTR)&blkObjID);
	}
...
int CNSDWGMgr::getBlockName(LONG_PTR nBlockObjID, NSSTRING &strBlockName)
{
	Acad::ErrorStatus es;
	AcDbObject *pEntity;
	AcDbObjectId pBlockObjID;
	...
	{
	pBlockObjID.setFromOldId((LONG_PTR)&nBlockObjID);
	}
	try{
		es = acdbOpenObject(pEntity, pBlockObjID, AcDb::kForRead);}
		catch(...)
...
int CNSDWGMgr::readBlockAttributes(LONG_PTR nBlockObjID, std::map<NSSTRING, NSSTRING> & mapOfAttributes)
{
	Acad::ErrorStatus es;
	AcDbObject *pEntity;
	AcDbObjectId pBlockObjID;
